...
$(document).ready(function () {
    ...
    $('.saveBtn').click(function () {
        ...
        if (match && id.startsWith('hour')) {
            const blockInput = $(this).siblings('.description').val();
            localStorage.setItem("hour" + match[0], blockInput);
            $(this).siblings('.description').val(blockInput); // Update the input field with the saved value
        }
    });
